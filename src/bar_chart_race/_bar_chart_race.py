...
def get_image_label(root_folder, name, size):
...
class _BarChartRace(CommonChart):
    ...
    def __init__(
        ...
        self.img_label_folder = (
            img_label_folder  # root folder where image labels are stored
        )
    ...
    def offset_image(self, location, lenght, name, ax):
        ...
        img = get_image_label(self.img_label_folder, img_name, self.img_label_size)
        im = OffsetImage(img, zoom=0.8)  # change zoom value based on icon image's size
        im.image.axes = ax
    ...
    def _add_tick_label_offset_image(self, location, length, name, ax):
        ...
        img_name = get_image_name(name)
        img = get_image_label(self.img_label_folder, img_name, self.img_label_size)
        im = OffsetImage(img, zoom=0.25)  # change zoom value based on icon image's size
        im.image.axes = ax
        ...
        if self.tick_image_mode == "trailing":  # images move along with the bar
            ...
            self.tick_image_mode == "fixed"
        ):  # images stay fixed at the beginning of the bar
            if self.orientation == "h":
    ...
    def prepare_axes(self, ax):
        ...
        ax.tick_params(
            axis="x",
            length=12,
            labelsize=self.tick_label_font["size"],
            pad=1,
            color="#F8FAFF",
        )
        ax.tick_params(
    ...
    def plot_bars(self, ax, i):
        ...
        if self.orientation == "h":
            ...
            ax.set_yticklabels(
                ax.get_yticklabels(), **self.tick_label_font, wrap=True
            )  # ,visible=False)
            # ax.set_yticklabels([])
        ...
        if (
            self.img_label_folder
        ):  # here I am handling the addition of images as the bar tick labels
    ...
    def add_bar_labels(self, ax, bar_location, bar_length):
        if self.bar_textposition:
            ...
            delta = 0.05 if self.bar_textposition == "outside" else -0.008
...
def bar_chart_race(
    ...
    img_label_size : int, default 100
        The size of image width, height.
