...
class _BarChartRace:
    ...
    def get_bar_kwargs(self, bar_kwargs):
        if bar_kwargs is None:
            ...
            if "opacity" not in bar_kwargs:
                bar_kwargs["opacity"] = 0.8
            return bar_kwargs
        raise TypeError(
            "`bar_kwargs` must be None or a dictionary mapping `go.Bar` parameters "
            "to values."
        )
    ...
    def get_layout_kwargs(self, layout_kwargs):
        if layout_kwargs is None:
            ...
            if {"xaxis", "yaxis", "annotations"} & layout_kwargs.keys():
                raise ValueError(
                    '`layout_kwargs` cannot contain "xaxis", "yaxis", or '
                    ' "annotations".'
                )
        ...
        raise TypeError(
            "`layout_kwargs` must be None, a dictionary mapping "
    ...
    def get_title(self, title):
        ...
        if isinstance(title, str):
            ...
            return title
        raise TypeError(
            "`title` must be a string, dictionary, or "
    ...
    def prepare_data(self, df):
        ...
        dfs = prepare_wide_data(
            ...
            sort=self.sort,
            n_bars=self.n_bars,
            interpolate_period=self.interpolate_period,
    ...
    def get_bar_colors(self, colors):
        ...
        if isinstance(colors, str):
            ...
            raise TypeError(
                "`colors` must be a string name of a colormap or " "sequence of colors."
            )
        ...
        if (
            bar_colors.dtype.kind == "f"
            and bar_colors.shape[1] == 3
            and (bar_colors <= 1).all()
        ):
    ...
    def set_fixed_max_limits(self):
        label_limit = (0.2, self.n_bars + 0.8)
        value_limit = None
    ...
    def get_frames(self):
        ...
        for i in range(len(self.df_values)):
            ...
            bar_locs = (
                bar_locs + np.random.rand(len(bar_locs)) / 10_000
            )  # done to prevent stacking of bars
            x, y = (
                (bar_vals, bar_locs)
                if self.orientation == "h"
                else (bar_locs, bar_vals)
            )
            ...
            label_axis["range"] = (
                self.ylimit if self.orientation == "h" else self.xlimit
            )
            ...
            value_axis["range"] = (
                self.xlimit if self.orientation == "h" else self.ylimit
            )
    ...
    def get_perpendicular_bar(self, bar_vals, i, layout):
        ...
        if self.fixed_max:
            delta = (value_limit[1] - value_limit[0]) * 0.02
        else:
            delta = (1.05 * bar_vals.max() - bar_vals.min()) * 0.02
