...
from ._utils import prepare_long_data as pld
...
_BCR.bar_chart_race.__doc__ = re.sub(
    "df : .*(?=filename :)", "", bcr.__doc__, flags=re.S
)
_BCR.line_chart_race.__doc__ = re.sub(
    "df : .*(?=filename :)", "", lcr.__doc__, flags=re.S
)
_BCR.prepare_wide_data.__doc__ = re.sub(
    "df : .*(?=filename :)", "", pwd.__doc__, flags=re.S
)
_BCR.prepare_long_data.__doc__ = re.sub(
...
if importlib.util.find_spec("plotly"):
    ...
    _BCR.bar_chart_race_plotly.__doc__ = re.sub(
        "df : .*(?=filename :)", "", bcrp.__doc__, flags=re.S
    )
